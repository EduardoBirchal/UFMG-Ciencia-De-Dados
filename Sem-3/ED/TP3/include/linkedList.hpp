...
private:
    listNode<T>* head;
    listNode<T>* tail; // Ponteiro para a cauda da lista
    int size;
...
linkedList<T>::linkedList(bool doDeallocateNodes) 
    : head(nullptr), tail(nullptr), size(0), deallocateNodes(doDeallocateNodes) {
}
...
void linkedList<T>::addToHead(T data) {
    ...
    head = newNode;
    if (tail == nullptr) { // Se a lista estava vazia, a cauda é o novo nó
        tail = newNode;
    }
    size++;
}

// Adiciona na cauda (Otimizado com o ponteiro tail)
template<class T>
...
void linkedList<T>::addOrdered(T data) {
    ...
    if (isEmpty()) {
        ...
        if (newNode->getNext() == nullptr) { // Se o novo nó é o último, atualiza a cauda
            tail = newNode;
        }
    }
    size++;
...
bool linkedList<T>::remove(int index) {
    ...
    if (index == 0) {
        alvo = head;
        head = head->getNext();
        if (head == nullptr) { // Se a lista ficou vazia
            tail = nullptr;
        }
    } else {
        ...
        if (antecessorAlvo->getNext() == nullptr) { // Se o nó removido era a cauda
            tail = antecessorAlvo;
        }
    }
